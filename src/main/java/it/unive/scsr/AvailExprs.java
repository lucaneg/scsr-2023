...
public class AvailExprs implements DataflowElement<DefiniteForwardDataflowDomain<AvailExprs>, AvailExprs> {
    ...
    //This function gen is used when there is an assignment where the variable on the left side
    //is not contained on the right side
    @Override
    public Collection<AvailExprs> gen(Identifier id, ValueExpression expression, ProgramPoint pp, DefiniteForwardDataflowDomain<AvailExprs> domain) throws SemanticException {
        ...
        //check if the variable (Identifier) on the left side is not contained on the right side
        if (!ci.contains(id)) {
    ...
    //This function kill is used when there is an assignment, and the variable on the left side
    //is contained on the right side
    @Override
    public Collection<AvailExprs> kill(Identifier id, ValueExpression expression, ProgramPoint pp, DefiniteForwardDataflowDomain<AvailExprs> domain) throws SemanticException {
        ...
        for (AvailExprs ae : df) {
            Collection<Identifier> ci = ae.getInvolvedIdentifiers();
            //check if the variable (Identifier) on the left side is contained on the right side
            if (ci.contains(id)) {
                res.add(ae);
            }
        }
    ...
    //This function kill is used when there is no variable to kill, there is no assignment
    @Override
