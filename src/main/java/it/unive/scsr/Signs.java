...
public class Signs extends BaseNonRelationalValueDomain<Signs> {
	 ...
	 */
	public static final Signs POS = new Signs((byte) 4);

	/**
	 * The abstract negative element.
	 */
	public static final Signs NEG = new Signs((byte) 3);

	/**
	 * The abstract zero element.
	 */
	public static final Signs ZERO = new Signs((byte) 2);

	/**
	 * The abstract top element.
	 */
	public static final Signs TOP = new Signs((byte) 0);

	/**
	 * The abstract bottom element.
	 */
	public static final Signs BOTTOM = new Signs((byte) 1);
	...
	@Override
	public Signs evalNullConstant(ProgramPoint pp) {
		return top();
	}

	@Override
	public Signs evalNonNullConstant(Constant constant, ProgramPoint pp) {
	...
	@Override
	public Signs evalUnaryExpression(UnaryOperator operator, Signs arg, ProgramPoint pp) {
	...
	@Override
	public Satisfiability satisfiesBinaryExpression(BinaryOperator operator, Signs left, Signs right,
													ProgramPoint pp) {
	 ...
	 */
	public Satisfiability eq(Signs other) {
	 ...
	 * @return the satisfiability of {@code this > other}
	 */
	public Satisfiability gt(Signs other) {
		if (this.equals(other))
	...
	@Override
	public Satisfiability satisfiesTernaryExpression(TernaryOperator operator, Signs left, Signs middle, Signs right,
													 ProgramPoint pp) {
		return Satisfiability.UNKNOWN;
	}

	@Override
	public ValueEnvironment<Signs> assumeBinaryExpression(
			ValueEnvironment<Signs> environment, BinaryOperator operator, ValueExpression left,
		...
		if (operator == ComparisonEq.INSTANCE)
			...
			if (left instanceof Identifier) {
				Signs rightSign = eval(right, environment, pp);
				if (rightSign.isPositive())
					environment = environment.assign((Identifier) left, right, pp);
			} else if (right instanceof Identifier) {
				Signs leftSign = eval(left, environment, pp);
