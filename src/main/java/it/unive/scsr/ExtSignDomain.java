...
public class ExtSignDomain extends BaseNonRelationalValueDomain<ExtSignDomain> {
    ...
    public ExtSignDomain() { this( 10 ); }
    public ExtSignDomain( int sign ) { this.sign = sign; }
    ...
    private ExtSignDomain evalAddition( ExtSignDomain left, ExtSignDomain right) {
        ...
        if ( left == ZERO ) {
            if ( right == ZERO || right == POSITIVE || right == ZERO_OR_POSITIVE || right == ZERO_OR_NEGATIVE || right == NEGATIVE )
                return right;
        }
        return TOP;
    }
    ...
    @Override
    public ExtSignDomain evalTernaryExpression(TernaryOperator operator, ExtSignDomain left, ExtSignDomain middle, ExtSignDomain right, ProgramPoint pp) throws SemanticException {
